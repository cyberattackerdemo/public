...
LOG_FILE=/home/troubleshoot/step3_restore_proxy_for_ubuntu_24.04.log
echo "$(date '+%Y-%m-%d %H:%M:%S') | ===== Starting step3_restore_proxy_for_ubuntu_24.04.sh =====" | tee $LOG_FILE
...
echo "$(date '+%Y-%m-%d %H:%M:%S') | ===== step3_restore_proxy_for_ubuntu_24.04.sh completed =====" | tee -a $LOG_FILE

# Proxyキャプチャ
echo "$(date '+%Y-%m-%d %H:%M:%S') | Capturing proxy traffic (8080) with tcpdump for 300 sec..." | tee -a $LOG_FILE
sudo timeout 300 tcpdump -nnvvXS -i any port 8080 | tee /home/troubleshoot/step3_tcpdump_proxy_for_ubuntu_24.04.txt
echo "$(date '+%Y-%m-%d %H:%M:%S') | tcpdump proxy capture saved: /home/troubleshoot/step3_tcpdump_proxy_for_ubuntu_24.04.txt" | tee -a $LOG_FILE
